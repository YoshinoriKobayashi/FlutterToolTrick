...
//
  ...
  // offline_accessスコープでAuthorization Codeフローを開始しました。これは、認証時にトークン・エンドポイントから返されるリフレッシュ・トークンが追加されていることを意味します。
  // リフレッシュ・トークンを使うと、ユーザーが認可サーバーにサインインしていなくても、新しいアクセス・トークンやIDトークンを取得できます。リフレッシュ・トークンを使用することで、ユーザーがアプリを起動するたびに再認証する必要はありません。その代わり、利用可能なリフレッシュ・トークンがあれば、それを使って新しいアクセストークンを静かに取得することができます。
  // flutter_secure_storageは、機密性の高いアプリケーションデータを保存・取得するためのシンプルなCRUD操作を公開するライブラリです。
  // アプリケーションを起動すると、initState()メソッドは、既存のリフレッシュトークンがあるかどうかをチェックします。既存のリフレッシュ・トークンがある場合は、appAuth.token()メソッドを呼び出して新しいアクセストークンを取得しようとします。以下は、_MyAppStateクラスに追加するメソッドです。
  @override
  void initState() {
    print("◆◆◆initState");
    initAction();
